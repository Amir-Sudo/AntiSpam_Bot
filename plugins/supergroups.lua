...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    ...
    save_data(_config.moderation.data, data)
    return '🔐قـفـل لـیـنـڪ در سـوپـرگـروه فـعـال شـد🔒'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return '🔐قــفــل اسـپـم در سـوپـرگـروه از قـبـل فـعـال بـود🔒'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔐قــفــل اسـپـم در سـوپـرگـروه فـعـال شـد🔒'
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return '🔓قــفــل اسـپـم در سـوپرگـروه از قـبـل غـیـرفـعـال بود🔓'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return '🔓قــفــل اسـپـم در سـوپـرگـروه غـیـرفـعـال شد🔓'
  end
end
...
local function lock_group_fwd(msg, data, target)
  ...
  if group_fosh_lock == 'yes' then
    return '🔐قـفـل فـوروارد در سـوپـرگـروه از قـبـل فـعـال بـود🔒'
  else
    data[tostring(target)]['settings']['lock_fwd'] = 'yes'
    save_data(_config.moderation.data, data)
    return '🔐قـفـل فـوروارد در سـوپـرگـروه فـعـال شـد🔒'
  end
end
...
local function lock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'yes' then
    return '🔐قـفـل ریــپلـی در سـوپـرگـروه از قـبـل فـعـال بـود🔒'
  else
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, '✅مـــکــــس پـــاور در گــــروه فــــعـــال شــــد✅خــــوش اومـــــدم🌸', ok_cb, false)
			end
		...
		if matches[1] == 'rem' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, '⭕مــکــس پـــاور در گــــروه غــــیـــرفــــعــال شــــد⭕کـــص عـــمـــتــون🚫', ok_cb, false)
			end
		...
		if matches[1] == 'id' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return "اسـم شـما: " ..string.gsub(msg.from.print_name, "_", " ").. "\n\nیـوزرنیـم شـما: @"..(msg.from.username or '----').."\n\nایـدے شـما: "..msg.from.id.."\n\nنـام سـوپـرگـروه: " ..string.gsub(msg.to.print_name, "_", " ").. "\n\nایـدے سـوپـرگـروه: "..msg.to.id
			end
		end
		...
		if msg.text then
			if msg.text:match("^(https://telegram.me/joinchat/%S+)$") and data[tostring(msg.to.id)]['settings']['set_link'] == 'waiting' and is_owner(msg) then
				...
				save_data(_config.moderation.data, data)
				return "🔁لـیـنـڪ جـدیـد سـاخـتـہ شـد"
			end
		end
		...
		if matches[1] == 'unmute' and is_momod(msg) then
			...
			local msg_type = 'Video'
				if is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return msg_type.." has been unmuted"m
				else
					return "Mute "..msg_type.." is already off"
				end
			end
		...
		if matches[1] == 'help' and not is_momod(msg) then
			text = "بـرای دریـافـتـہ دسـتـورات فارسـیـہ پـاور اسـپـیـد به کـانـال @MaxPowerTM مـراجـعـہ ڪـنـیـد."
